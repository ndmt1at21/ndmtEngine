...
namespace mt
{
	...
	const RectInt Rectangle::getLocalBound() const
	{
		return m_bound;
	}

	const RectInt Rectangle::getGlobalBound() const
	{
		RectInt rect;
	...
	void Rectangle::draw(ndmtEngine* target) const
	{
		if (m_bFill)
			fill(target);
		else
	...
	void Rectangle::drawOutline(ndmtEngine* target) const
	{
	...
	void Rectangle::fill(ndmtEngine* target) const
	{
