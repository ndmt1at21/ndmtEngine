...
namespace mt
{
	...
	const RectInt Circle::getLocalBound()
	{
		return m_bound;
	}

	const RectInt Circle::getGlobalBound()
	{
		RectInt rect;
	...
	void Circle::draw(ndmtEngine* target)
	{
		if (m_bFill)
			drawOutline(target);
		else
	...
	void Circle::drawOutline(ndmtEngine* target)
	{
		// C
	...
	void Circle::fill(ndmtEngine* target)
	{
		// C
		int xc = m_radius + m_position.x;
		int yc = m_radius + m_position.y;
